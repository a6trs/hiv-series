...
c.getContentSize().width;c.setProgress(d)}},onTouchEnded:function(){acg.ext._cp_shouldbepaused||acg.resume();acg.ext._cp_shouldbepaused=null}},b);return b};acg.ext.pushArcVerts=function(a,b,c,d,e){e=(e-d)/25;for(var f=0;25>=f;f++){var g=f*e;a.push(cc.p(c*Math.cos(g+d)+b.x,c*Math.sin(g+d)+b.y))}};
...
0;b<e;b++)f.getChildByTag(b).setVisible(b===a)});f.setAnchorPoint(cc.p(0,1));f.setPosition(cc.p(0,-5));f.setContentSize(cc.size(60,45));f.setCascadeOpacityEnabled(!0);b.setCascadeOpacityEnabled(!0);b.addChild(f);for(c=0;c<e;c++)d=cc.Sprite.create(acg.ext._cp_danmakutypes[c]),d.setAnchorPoint(cc.p(0,0)),d.setVisible(0===c),f.addChild(d,0,c);var g=acg.ext.toggle_menu(acg.ext._cp_textcolours.length,function(a){g.getChildByTag(acg.ext._cp_colourdisp_tag).setColor(cc.color(acg.ext._cp_textcolours[a]))});
g.setAnchorPoint(cc.p(0,1));g.setPosition(cc.p(75,-6));g.setContentSize(cc.size(60,45));g.setCascadeOpacityEnabled(!0);b.addChild(g);c=cc.LayerColor.create(cc.color(0,0,0));c.setContentSize(cc.size(54,40.5));c.setPosition(cc.p(0,3));g.addChild(c,0);c=cc.LayerColor.create(cc.color(acg.ext._cp_textcolours[0]));c.setContentSize(cc.size(48,36));c.setPosition(cc.p(3,6));g.addChild(c,1,acg.ext._cp_colourdisp_tag);cc.eventManager.addListener({event:cc.EventListener.TOUCH_ONE_BY_ONE,swallowTouches:!0,onTouchBegan:function(c,
d){var e=acg.ext._cp_ctrls_showed&&acg.ext.is_touch_in_content(c,d);if(e){var m=document.getElementById("acg_cp_danmaku"),q=document.getElementById("Cocos2dGameContainer");m.style.display="";m.style.position="absolute";m.style.top="5%";m.style.left=q.offsetLeft+.06*q.clientWidth+"px";m.style.width=.88*q.clientWidth+"px";m.style["font-size"]=16/acg.height*q.clientHeight+"px";setTimeout(function(){m.focus()},10);m.onkeyup=function(c){13===c.keyCode&&(a(acg.time,f.selectedIndex(),acg.ext._cp_textcolours[g.selectedIndex()],
c.target.value),c.target.value="",b.hideInput())}}return e}},b);b.hideInput=function(){document.getElementById("acg_cp_danmaku").style.display="none"};return b};
...
acg.ext._233_addSliding=function(a,b,c,d,e){e=e||acg.colour.WHITE;b=cc.LabelTTF.create(b,acg.ext._233_fontname,acg.ext._233_fontsize);b.setAnchorPoint(cc.p(0,1));b.setNormalizedPositionX(1);b.setNormalizedPositionY(1-a*acg.ext._233_lineh/acg.height);b.setColor(e);b.setVisible(!1);b.runAction(cc.sequence(cc.delayTime(c),cc.show(),cc.moveBy(d,cc.p(-(1+b.getContentSize().width/acg.width),0)),cc.removeSelf()));acg.ext._233_layer.addChild(b)};
acg.ext._233_addSticky=function(a,b,c,d,e,f){f=f||acg.colour.WHITE;c=cc.LabelTTF.create(c,acg.ext._233_fontname,acg.ext._233_fontsize);c.setLocalZOrder(2333333);if(1===a)c.setAnchorPoint(cc.p(.5,1)),c.setNormalizedPosition(cc.p(.5,1-b*acg.ext._233_lineh/acg.height));else if(2===a)c.setAnchorPoint(cc.p(.5,0)),c.setNormalizedPosition(cc.p(.5,b*acg.ext._233_lineh/acg.height));else{console.log("acg.ext._233_addSticky: Invalid type T^T");return}c.setColor(f);c.setVisible(!1);c.runAction(cc.sequence(cc.delayTime(d),
cc.show(),cc.delayTime(e),cc.removeSelf()));acg.ext._233_layer.addChild(c)};acg=acg||{};acg.ext=acg.ext||{};acg.ext._background_flow=[];acg.ext._background_id=null;acg.ext.background=function(a,b,c){acg.ext._background_flow.push({time:a+acg._cur_offset,colour:b,duration:c||.2})};
...
acg.rect({x:0,y:0,colour:[0,0,0]},a);acg.put(0,acg.ext._background_id)};
